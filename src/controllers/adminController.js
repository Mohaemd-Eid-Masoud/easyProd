...
,
  ...
  try {
    ...
    // Delete directory associated with the user
    const userDirPath = path.join(process.cwd(), './src/uploads/', user.dirName,`${user.username}-template/${templatename}`);
...
geTemplates: async (req, res) => {
  try {
    ...
    const templateUrls = imagePaths.map(imagePath => {
      return `http://${req.get('host')}/${user.dirName}/${user.username}-template/${imagePath}`;
    });
 ...
 getAvatars : async(req,res) => {
  ...
  const host = req.get('host');
   const avatars = await upload.getAllImageUrls(username,host); 
   res.json ({avatars});
 }
};
