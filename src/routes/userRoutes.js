...
const upload = require('../middleware/uploadMiddleware'); // No need to specify a destination for Multer
...
const getTemplateUrls = async (userId, host) => {
  try {
    ...
    for (const imagePath of imagePaths) {
      const imageUrl = `http://${host}/${user.dirName}/${user.username}-template/${imagePath}`;
...
// Route for uploading image with title and description
// Middleware for image upload
router.post('/upload', upload.upload.single('img'), async (req, res, next) => {
  try {    
          ...
          // Validate image upload data including title and description
          const { errors, isValid } = validateImageUploadData.validateImageUploadData({ image });
          if (!isValid) {
            return res.status(400).json(errors);
          }
         const userData = {username,image};
         upload.saveAvatar(userData)
